    ...
    <div class="slide-1 home-slider">
        ...
        foreach ($rows as $row) {
            ...
            <div class="home  text-center">
                <img src="assets/img/home-banner/' . $row['img'] . '" alt="' . $row['title'] . '" class="bg-img blur-up lazyload">
                <div class="container">
                    <div class="row">
                        <div class="col">
                            <div class="slider-contain">
                                <div>
                                    <h4>' . $row['text'] . '</h4>
                                    <h1>' . $row['title'] . '</h1>
                                    <a href="' . $row['link'] . '" class="btn btn-solid">shop now</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
<section class="pb-0 ratio2_1">
    <div class="container">
        <div class="row partition2">
            ...
            $stmt = $con->prepare("SELECT * FROM prod_category WHERE status != 0 LIMIT 2");
            $stmt->execute();
            $rows = $stmt->fetchAll();
            ...
            foreach ($rows as $row) {
                echo '
                <div class="col-md-6">
                <a href="shop.php?filter=1&category=' . $row['category'] . '">
                    <div class="collection-banner p-right text-center">
                        <div class="img-part">
                            <img src="assets/img/categories/' . $row['img'] . '" class="img-fluid blur-up lazyload bg-img" alt="' . $row['category'] . '">
                        </div>
        ...
        <div class="row">
            <div class="col">
                <div class="product-4 product-m no-arrow">
                    <?php
                    $stmt = $con->prepare("SELECT * FROM products WHERE quantity != 0 ORDER BY id DESC LIMIT 6");
                    $stmt->execute();
                    $rows = $stmt->fetchAll();
...
if (count($rows) > 0) {
    foreach ($rows as $row) {
        echo '
        <section class="p-0">
        <div class="full-banner parallax text-center p-left" style=\'background-image: url("assets/img/promo-images/' . $row['img'] . '");background-size: cover;
        background-position: center center;
        display: block;\'>
            <img src="assets/img/promo-images/' . $row['img'] . '" alt="' . $row['img'] . '" class=" bg-img blur-up lazyload">
        </div>
    </section>
        ';
    }
            ...
            <div class="col">
                <div class="theme-tab">
                    <ul class="tabs tab-title">
                        ...
                        <?php
                        $stmt = $con->prepare("SELECT category FROM prod_category WHERE status != 0 LIMIT 3");
                        $stmt->execute();
                        $rows = $stmt->fetchAll();
                        $tab_number = 4;
                        foreach ($rows as $row) {
                            echo '<li class=""><a href="tab-' . $tab_number++ . '">' . $row['category'] . '</a></li>';
                        }
                        ?>
                    </ul>
                    ...
                    foreach ($categories as $category) {
                        echo '
                                    <div id="tab-' . $tab_number++ . '" class="tab-content ">
                                        <div class="no-slider row">
                                    ';
                        $stmt = $con->prepare("SELECT * FROM products WHERE category = ? AND quantity != 0 ORDER BY id DESC LIMIT 4");
...
<script>
    $('.tab-title li:first-child').addClass('current');
    $('.tab-content-cls .tab-content:first-child').show();
